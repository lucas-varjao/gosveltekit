...
<section class="flex justify-center items-center min-h-[calc(100vh-6rem)] py-12 px-4">
	<div class="w-full max-w-lg bg-slate-900 rounded border border-slate-800 shadow-lg p-8">
		<!-- Using flexbox for vertical content alignment -->
		<div class="flex flex-col gap-6">
			<h1 class="text-3xl font-bold text-center text-white">Create an account</h1>
			...
			<form onsubmit={handleSubmit} class="flex flex-col gap-4">
				...
				<div class="flex flex-col gap-2">
					...
					{#if errors.confirmPassword && touched.confirmPassword}
						<p transition:slide class="text-sm text-red-500">{errors.confirmPassword}</p>
					{/if}
				</div>
